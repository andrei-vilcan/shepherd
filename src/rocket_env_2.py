"""JAX implementation of rocket landing environment with orbital mechanics."""
...
class EnvParams(environment.EnvParams):
    ...
    init_min_orbit_radius: float = 10.0
...
class RocketLander(environment.Environment[EnvState, EnvParams]):
    ...
    def reset_env(
        ...
        orbit_velocity_magnitude_init += jax.random.uniform(orbit_vel_noise_key)

        dx_init = -orbit_velocity_magnitude_init * jnp.cos(angle_init)  # negative for counterclockwise orbit
        dy_init = orbit_velocity_magnitude_init * jnp.sin(angle_init)
    ...
    def get_obs(self, state: EnvState, params: EnvParams=None, key: jax.Array=None) -> jax.Array:
        ...
        alpha = jnp.arctan2(state.y, state.x)
        theta_relative = state.theta - alpha
    ...
    def is_terminal(self, state: EnvState, params: EnvParams) -> jax.Array:
        ...
        escaped = r > params.planet_radius * 5
    ...
    def _compute_reward(self, old_state: EnvState, new_state: EnvState, params: EnvParams) -> jax.Array:
        ...
        i_cl = landed and slow and correct_position and correct_orientation and low_spin
        reward = jnp.where(i_cl, params.reward_i_cl, 0.0)

        # non-ideal landing at correct location
        ni_cl = landed and correct_position and not (slow and correct_orientation and low_spin)
        reward = jnp.where(ni_cl, params.reward_ni_cl, 0.0)

        # non-ideal landing at incorrect location
        ni_il = landed and not correct_position and not(slow and correct_orientation and low_spin)
    ...
    def observation_space(self, params: Optional[EnvParams] = None) -> spaces.Box:
        ...
        low = jnp.array([-1.0, -jnp.pi, -5.0, -5.0, -1.0, -1.0, -10.0, 0.0, -1.0, 0.0])
        high = jnp.array([5.0, jnp.pi, 5.0, 5.0, 1.0, 1.0, 10.0, 1.0, 1.0, 1.0])
        return spaces.Box(low, high, shape=(10,), dtype=jnp.float32)
